...
class ComponentList {
  ...
  add(component) {
    ...
    if (previousObjIndex !== -1) {
      ...
      _.each(['metadata', 'patches', 'extraFiles', 'source'], (k) => {
  ...
  _getComponent(componentSpec) {
    const obj = this._componentProvider.getComponent(componentSpec);
   ...
   */
  getPrintableList() {
    return _.map(this._components, component => `${component.metadata.id}@${component.version}`).join(', ');
  }
}
