...
class LogicProvider {
  ...
  loadBuildInstructions(recipePath) {
    ...
    _.each(this._componentTypeCollections, componentCollection => {
      ...
      if (typeof exportedTypes === 'function') {
        // The collection exposes just one component type
        sandbox[exportedTypes.name] = exportedTypes;
      } else {
        _.each(exportedTypes, (component, id) => {
          sandbox[id] = component;
        });
      }
    });
