...
class ContainerizedBuilder {
  ...
  _generateCmd(buildData, paths, prefix, options) {
    ...
    nfile.write(nfile.join(paths.config.hostPath, 'containerized-build.json'), JSON.stringify(buildData, null, 4));
    cmd.push(nfile.join(paths.config.containerPath, 'containerized-build.json'));
