...
class ContainerizedBuilder {
  ...
  build(buildData, baseImages, options) {
    ...
    if (options.buildDir) {
      ...
      const lastComponent = _.last(buildData.components);
      const buildTail = `${strftime('%F-%H%M%S')}-${lastComponent.id}-stack`;
      buildDir = nfile.join(output, buildTail);
    }

    this.logger.debug('Preparing build environment');
    ...
    _.each(buildData.components, component => {
      if (component.source.tarball) {
        ...
        this.logger.trace(`Skipping ${component.id} tarball since it is not defined`);
      }
