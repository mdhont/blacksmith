...
class ContainerizedBuilder {
  ...
  _generateCmd(buildData, paths, prefix, options) {
    ...
    if (options.platform) cmd.push(`--platform=${options.platform}`);
    if (options.flavor) cmd.push(`--flavor=${options.flavor}`);
   ...
   * @param {string} [options.flavor=null] - Flavor of the build
   * @param {string} [options.platform='linux-x64'] - Platform to use
  ...
  build(buildData, imageId, options) {
    ...
    if (options.buildDir) {
      ...
      const buildId = _.last(this.blacksmithInstance.bm.getComponentsMetadata(buildData).components).id;
      const buildTail = `${strftime('%F-%H%M%S')}-${buildId}-${options.platform}-${options.flavor || 'standard'}`;
