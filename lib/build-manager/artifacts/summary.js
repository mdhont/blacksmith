...
class Summary {
  ...
  addArtifact(component, buildTime) {
    ...
    if (this._fsTracker) {
      const artifactId = `${component.metadata.id}-${component.metadata.version}` +
      `-${this.platform.os}-${this.platform.arch}`;
  ...
  */
  serialize(directory) {
    ...
    const summaryFileId = `${this.id}-${this.platform.os}-${this.platform.arch}`;
