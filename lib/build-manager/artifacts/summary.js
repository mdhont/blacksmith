 ...
 * @property {string} flavor - Flavor of the build
...
class Summary {
  ...
  addArtifact(component, buildTime) {
    ...
    if (this._fsTracker) {
      const artifactId = `${component.metadata.id}-${component.metadata.version}-${this._be.target.platform}`;
      ...
      this._fsTracker.captureDelta(artifactTarball, {prefix: component.prefix, pick: component.pick});
      ...
      if (!_.isEmpty(commit) && nfile.exists(artifactTarball)) {
        ...
        compiledTarball.md5 = ncrypt.md5({file: artifactTarball});
      }
    }
  ...
  */
  serialize(directory) {
    ...
    let summaryFileId = `${this.id}-${this.platform}`;
    if (this.flavor) summaryFileId += `-${this.flavor}`;
    const tarFileTail = `${summaryFileId}.tar.gz`;
    const tarFile = nfile.join(directory, tarFileTail);
    this.compressArtifacts(tarFile);
    const md5 = ncrypt.md5({file: tarFile});
    nfile.write(nfile.join(directory, `${summaryFileId}-build.json`), this.toJson({tarball: tarFileTail, md5: md5}));
  }
}
