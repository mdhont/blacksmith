...
class BuildManager {
   ...
   * @param {Object} [options.flavor] - Build flavor
   * @returns {@link BuildEnvironment}
   */
  createBuildEnvironment(options) {
    options = _.opts(options, {platform: 'linux-x64', flavor: null});
   ...
   * @param {string} [buildData.platform='linux-64'] - Platform of the build
   ...
   * @param {Object} [options.platform] - Build platform
   * @param {Object} [options.flavor] - Build flavor
   * @returns {Object} - Object defining the platform, flavor, and components metadata
   * @example
   * getComponentsMetadata(['zlib']);
   * // => { platform: 'linux-x64',
   * //      flavor: null,
   * //      components:
  ...
  getComponentsMetadata(buildData, options) {
    options = _.opts(options, {platform: null, flavor: null, logger: this.logger});
    ...
    if (_.isReallyObject(buildData)) {
      ...
      const optionsFromSpec = _.pick(buildData, ['platform', 'flavor']);
      _.each(optionsFromSpec, (v, k) => {
        if (_.isEmpty(options[k])) options[k] = v;
      });
    }
    ...
    return {platform: this.be.target.platform, flavor: this.be.target.flavor, components: objList};
  }
   ...
   * @param {string} [options.flavor] - Compilation flavor. F.e. 'alpine'
   * @param {string} [options.platform='linux-x64'] - Platform of the build
   * @example
  ...
  build(buildData, options) {
    ...
    if (_.isReallyObject(buildData)) {
      ...
      const flavor = options.flavor || buildData.flavor;
      options = _.opts({platform: platform, flavor: flavor, buildId: buildId}, options);
    } else {
  ...
  _buildComponents(components, options) {
    options = _.opts(options, {platform: 'linux-x64', abortOnError: true, forceRebuild: false,
    continueAt: null, incrementalTracking: false, buildId: null, flavor: null, logger: this.logger});
    ...
    if (!options.buildDir) {
      const buildTail = `${strftime('%F-%H%M%S')}-${options.buildId}-${this.be.target.platform}` +
      `-${this.be.target.flavor || 'standard'}`;
    ...
    this.logger.info(`Building for target ${JSON.stringify(_.pick(this.be.target, ['platform', 'flavor']))}`);
