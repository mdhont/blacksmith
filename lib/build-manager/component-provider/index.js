...
class ComponentProvider {
  ...
  getRecipe(id, requirements) {
    ...
    return new Recipe(id, this._obtainMetadata(id, requirements, recipeDir), {
    ...
    if (result === null) {
      if (_.isEmpty(requirement) ||
        ...
        if (!_.isUndefined(availableBranches)) {
          result = versionUtils.getVersionFromRange(availableBranches, requirement);
        } else {
  ...
  parseComponentReference(reference) {
    ...
    if (_.isString(reference)) {
      ...
      sourceTarball = reference.sourceTarball;
      id = reference.id;
