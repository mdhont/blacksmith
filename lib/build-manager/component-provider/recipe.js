...
class Recipe {
  ...
  _obtainMetadata(id, requirements, path) {
    ...
    result = JSON.parse(nhb.renderText(JSON.stringify(result),
  ...
  _loadRecipeJS(file) {
    ...
    const sandbox = {
      ...
      $loadBuildInstructions: (id) => {
        return this.loadBuildInstructions(id, this._recipeDirectories);
      },
      _: _,
      path: require('path'),
      require: function(p) {
  ...
  _getRecipeClass(id, requirements) {
    ...
    const recipe = this.loadBuildInstructions(id, this._recipeDirectories);
