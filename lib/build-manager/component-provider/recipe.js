...
const MakeComponent = require('blacksmith-base-components').MakeComponent;
...
class Spec {
  constructor(specHash) {
    specHash = _.opts(specHash, {version: null, platforms: []});
  ...
  satisfies(requirements) {
    ...
    _.each(requirements, (value, key) => {
      switch (key) {
        case 'version': {
          const cleanedRequiredVersion = getSemanticVersion(value);
          if (this.version !== null
              && !semver.satisfies(cleanedRequiredVersion, versionRangeWithPreRelease(this.version))) {
            satisfies = false;
          }
          break;
        }
  ...
  // Shorthand for manually instantiating for each and compare
  static findMatchingSpec(specList, requirements) {
...
class Recipe {
   ...
   * @function Recipe~validateMetadata
