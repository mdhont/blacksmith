...
class ComponentList {
  ...
  add(component) {
    ...
    if (previousObjIndex !== -1) {
      _.each(['metadata', 'patches', 'extraFiles', 'sourceTarball'], (k) => { // Override component properties that
        if (!_.isEmpty(_component[k])) {                                      // may have changed
          this._components[previousObjIndex][k] = _component[k];
        }
      });
    } else {
      // Component is new
  ...
  _getComponent(componentSpec) {
    ...
    const obj = this._componentProvider.getComponent(componentData, {platform: this.be.target.platform});
